 ...
 *
 * @author ag777
 * @version create on 2019年08月01日,last modify at 2025年06月15日
 */
public class Zip4jUtils {
	 ...
	 */
	public static List<String> getAllFilePaths(File file, String password) throws ZipException {
		ZipFile zipFile = getZipFile(file.getAbsolutePath(), password);
	 ...
	 * @throws ZipException 解析异常
	 */
	public static List<String> getAllFilePaths(String zipPath, String password) throws ZipException {
		ZipFile zipFile = getZipFile(zipPath, password);
		return zipFile.getFileHeaders().stream()
				.map(FileHeader::getFileName)
				.collect(Collectors.toList());
	}
	 ...
	 * @throws ZipException 解析异常
	 */
	public static List<FileHeader> getAllFileHeaders(File file, String password) throws ZipException {
		ZipFile zipFile = getZipFile(file.getAbsolutePath(), password);
	 ...
	 * @throws ZipException 解析异常
	 */
	public static List<FileHeader> getAllFileHeaders(String zipPath, String password) throws ZipException {
		ZipFile zipFile = getZipFile(zipPath, password);
		return zipFile.getFileHeaders();
	}
	
	/**
	 * 获取压缩包中只包含文件（不包含目录）的路径列表
	 * 
	 * @param file 压缩文件
	 * @param password 密码，可以为null
	 * @return 文件路径列表（不包含目录）
	 * @throws ZipException 解析异常
	 */
	public static List<String> getFilePathsOnly(File file, String password) throws ZipException {
		ZipFile zipFile = getZipFile(file.getAbsolutePath(), password);
		return zipFile.getFileHeaders().stream()
	 ...
	 * @throws ZipException 解析异常
	 */
	public static List<String> getFilePathsOnly(String zipPath, String password) throws ZipException {
		ZipFile zipFile = getZipFile(zipPath, password);
		return zipFile.getFileHeaders().stream()
				.filter(header -> !header.isDirectory())
				.map(FileHeader::getFileName)
	 ...
	 * @param zipPath 压缩文件路径
	 * @param password 密码，可以为null
	 * @param filePaths 要解压的文件路径列表（在压缩包内的相对路径）
	 * @param destinationDir 解压目标目录
	 * @throws ZipException 解压异常
	 */
	public static void extractFiles(String zipPath, String password, List<String> filePaths, String destinationDir) throws ZipException {
		ZipFile zipFile = getZipFile(zipPath, password);
		
		for (String filePath : filePaths) {
			zipFile.extractFile(filePath, destinationDir);
		}
	}
	 ...
	 * @throws ZipException 解压异常
	 */
	public static void extractFiles(File file, String password, List<String> filePaths, String destinationDir) throws ZipException {
		extractFiles(file.getAbsolutePath(), password, filePaths, destinationDir);
	}
	
	/**
	 * 根据FileHeader列表选择性解压文件
	 * 
	 * @param zipPath 压缩文件路径
	 * @param password 密码，可以为null
	 * @param fileHeaders 要解压的FileHeader列表
	 * @param destinationDir 解压目标目录
	 * @throws ZipException 解压异常
	 */
	public static void extractFilesByHeaders(String zipPath, String password, List<FileHeader> fileHeaders, String destinationDir) throws ZipException {
		ZipFile zipFile = getZipFile(zipPath, password);
		
		for (FileHeader fileHeader : fileHeaders) {
			zipFile.extractFile(fileHeader, destinationDir);
		}
	}
	 ...
	 * @throws ZipException 解压异常
	 */
	public static void extractFilesByHeaders(File file, String password, List<FileHeader> fileHeaders, String destinationDir) throws ZipException {
		extractFilesByHeaders(file.getAbsolutePath(), password, fileHeaders, destinationDir);
	}
	
	/**
	 * 根据文件路径选择性解压单个文件，并可以重命名
	 * 
	 * @param zipPath 压缩文件路径
	 * @param password 密码，可以为null
	 * @param filePath 要解压的文件路径（在压缩包内的相对路径）
	 * @param destinationDir 解压目标目录
	 * @param newFileName 新文件名，如果为null则保持原名
	 * @throws ZipException 解压异常
	 */
	public static void extractFile(String zipPath, String password, String filePath, String destinationDir, String newFileName) throws ZipException {
		ZipFile zipFile = getZipFile(zipPath, password);
		
		if (newFileName != null) {
			zipFile.extractFile(filePath, destinationDir, newFileName);
		} else {
			zipFile.extractFile(filePath, destinationDir);
		}
	}
	 ...
	 * @throws ZipException 解压异常
	 */
	public static void extractFile(String zipPath, String password, String filePath, String destinationDir) throws ZipException {
		extractFile(zipPath, password, filePath, destinationDir, null);
	}
	
	/**
	 * 根据文件路径模式选择性解压文件（支持通配符）
	 * 
	 * @param zipPath 压缩文件路径
	 * @param password 密码，可以为null
	 * @param pathPattern 文件路径模式（支持*和?通配符）
	 * @param destinationDir 解压目标目录
	 * @return 实际解压的文件数量
	 * @throws ZipException 解压异常
	 */
	public static int extractFilesByPattern(String zipPath, String password, String pathPattern, String destinationDir) throws ZipException {
		ZipFile zipFile = getZipFile(zipPath, password);
		...
		List<String> matchedPaths = allHeaders.stream()
				.map(FileHeader::getFileName)
				.filter(path -> path.matches(regex))
				.collect(Collectors.toList());
		
		extractFiles(zipPath, password, matchedPaths, destinationDir);
	 ...
	 */
	public static void unZipSingleFile(ZipFile zipFile, String absPath, String destinationPath) throws ZipException {
	 ...
	 * @throws IOException 如果处理Zip文件时发生错误。
	 */
	public static boolean testPassword(File file, String password) throws IOException {
	 ...
	 * @return 压缩文件
	 */
	public static ZipFile getZipFile(String zipPath, String password) {
	 ...
	 */
	public static Optional<ZipInputStream> getInputStream(ZipFile zipFile, String absPath) throws IOException {
		Optional<FileHeader> fileHeader = getFileHeader(zipFile, absPath);
		if(!fileHeader.isPresent()) {
