...
public class AiHttpClients {
    ...
    /**
     * 创建OpenAI兼容的AI HTTP客户端，并设置HTTP代理。
     *
     * @param baseUrl API基础URL，如 "https://api.openai.com"
     * @param apiKey API密钥，如 "sk-xxx"
     * @param proxyIp 代理服务器IP或域名
     * @param proxyPort 代理服务器端口
     * @return 配置完成的AI HTTP客户端
     * @throws IllegalArgumentException 如果baseUrl或apiKey为null或空
     */
    public static AiHttpClient openAiCompatible(String baseUrl, String apiKey, String proxyIp, int proxyPort) {
        requireNonBlank(baseUrl, "baseUrl");
        requireNonBlank(apiKey, "apiKey");
        return AiHttpClient.openAiCompatible(
                AiHttpClientConfig.create(baseUrl)
     ...
     * @throws IllegalArgumentException 如果任意参数为null或空
     */
    public static AiHttpChatSession openAiCompatibleSession(String baseUrl, String apiKey, String model, String proxyIp, int proxyPort) {
        requireNonBlank(model, "model");
        return AiHttpChatSession.of(openAiCompatible(baseUrl, apiKey, proxyIp, proxyPort), model);
    }
}
